document.addEventListener("DOMContentLoaded", function () {
  ...
  function processImage(file) {
    ...
    // Set available conversion options
  ...
  function setAvailableFormats(type) {
    const format = type.split("/")[1]; // e.g., jpeg
    const options = [
  ...
  function compressAndRenderImage() {
    if (!originalFile) {
    ...
    reader.onload = function (event) {
      ...
      img.onload = function () {
        ...
        canvas.toBlob(
          ...
          mimeType,
          1 // Full quality, since size is already reduced by scale
        );
      };
    };
